...
const CustomTools: React.FC = () => {
  const { customTools, plan, selectedProfessionId } = useSelector((state: RootState) => state.config);
  ...
  const canSelectMore = plan === 'pro' || customTools.length < FREE_TOOL_LIMIT;
    ...
    return (
      <div className="max-w-4xl mx-auto py-10">
        <div className="rounded-2xl border border-app bg-surface p-6">
          ...
          <button onClick={() => navigate('/setup')} className="mt-4 px-4 py-2 rounded-lg bg-primary-app text-white">Go to Setup</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto py-10">
        <div className="rounded-2xl border border-app bg-surface p-6">
          ...
          <p className="text-sm text-subtle mt-2">Free mode active. Upgrade to Pro from Profile to unlock extra custom tools.</p>
          <button onClick={() => navigate('/profile')} className="mt-4 px-4 py-2 rounded-lg bg-primary-app text-white">Go to Profile</button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto py-6 space-y-8">
      ...
      <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <p className="text-xs text-slate-500">
          ...
          {plan === 'free' && <span> | Free mode active, pro unlocks unlimited selection.</span>}
        </p>
        <button onClick={() => dispatch(setCustomTools([]))} className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-semibold text-slate-700">
          Reset Selection
        </button>
      </div>
    </div>
