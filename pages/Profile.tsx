...
import PaidInterestModal from '../components/PaidInterestModal';
...
const Profile: React.FC = () => {
  ...
  const submitPaidInterest = async (requestedPlan: 'pro' | 'business', submittedPhone?: string) => {
    ...
    if (!finalPhone) throw new Error('Phone number is required.');
    const owner = String(ownerName || '').trim();
    const business = String(shopName || '').trim();
    const email = String(authEmail || '').trim().toLowerCase();
    if (!owner || !business || !email) {
      throw new Error('Complete profile details (owner, business, email) before requesting paid plan.');
    }
  ...
  const handlePlanSelect = async (selected: PlanId) => {
    ...
    if (!isAdmin && nextPlan !== 'free') {
      ...
      try {
        ...
        if (!finalPhone) {
          setPendingPlan(nextPlan === 'business' ? 'business' : 'pro');
          emitToast({ variant: 'info', message: 'Phone number required for paid plan request.' });
          return;
        }
        await submitPaidInterest(nextPlan, finalPhone);
        emitToast({ variant: 'success', message: 'Plan request submitted. Our team will contact you.' });
      } catch (error: any) {
        emitToast({ variant: 'error', message: error?.message || 'Failed to submit paid plan request.' });
      } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-16">
      ...
      <section className="bg-surface border border-app rounded-2xl p-6">
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2">
          <input
            ...
            placeholder="Save phone number (e.g. 1234567890)"
          ...
          <button
            onClick={async () => {
              ...
              const sanitized = phoneDraft.trim();
              if (!sanitized) {
                emitToast({ variant: 'error', message: 'Phone number is required.' });
                return;
              }
              try {
                await savePlanAndPhone(authUid, normalizePlan(plan), sanitized);
                dispatch(setPhoneNumber(sanitized));
                emitToast({ variant: 'success', message: 'Phone number saved in profile.' });
              } catch (error: any) {
                emitToast({ variant: 'error', message: error?.message || 'Failed to save phone number.' });
              }
            }}
            className="px-4 py-2 rounded-xl bg-primary-app text-white font-semibold"
          >
            Save Phone
          </button>
        </div>
      ...
      <PaidInterestModal
        ...
        requestedPlan={pendingPlan || 'pro'}
        currentPhone={phoneNumber}
        onSubmitPhone={async (phoneValue) => {
          if (!pendingPlan || !authUid) throw new Error('Unable to submit plan request.');
          await submitPaidInterest(pendingPlan, phoneValue);
          emitToast({ variant: 'success', message: 'Plan request submitted successfully.' });
          setPendingPlan(null);
        }}
      />
    </div>
