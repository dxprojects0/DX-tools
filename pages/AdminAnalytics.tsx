...
const AdminAnalytics: React.FC = () => {
  ...
  const [data, setData] = useState<{
    ...
    users: Array<{ id: string; uid?: string; email: string; ownerName: string; shopName: string; category: string; plan: string; role?: string }>;
                    ...
                    <select
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          setData((prev) => {
                            if (!prev) return prev;
                            const nextUsers = prev.users.map((entry) => (entry.id === user.id ? { ...entry, plan: nextPlan } : entry));
              ...
              <div className="max-h-[38vh] overflow-y-auto space-y-2">
                {(data.paidInterestUsers || []).map((entry) => (
                  <div key={String(entry.requestId || entry.id)} className="border border-app rounded-xl p-3">
                    <p className="font-semibold text-sm">{String(entry.businessName || entry.shopName || '-')}</p>
                    <p className="text-xs text-subtle">{String(entry.ownerName || '-')} | {String(entry.email || '-')}</p>
                    <p className="text-xs mt-1">{String(entry.phoneNumber || '-')} | Plan: {String(entry.requestedPlan || '-')}</p>
                  </div>
                ))}
              ...
              <div className="max-h-[38vh] overflow-y-auto space-y-2">
                {(data.paidUsersSummary || []).map((entry) => (
                  <div key={String(entry.uid || entry.id)} className="border border-app rounded-xl p-3">
                    <p className="font-semibold text-sm">{String(entry.businessName || '-')}</p>
                    <p className="text-xs text-subtle">{String(entry.ownerName || '-')} | {String(entry.email || '-')}</p>
                    <p className="text-xs mt-1">{String(entry.phoneNumber || '-')} | Plan: {String(entry.plan || 'free')}</p>
                  </div>
                ))}
