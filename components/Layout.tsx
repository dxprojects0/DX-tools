...
import { BarChart3, Clock3, Home, ListChecks, LogOut, Menu, Plus, ReceiptText, UserCircle2, X } from 'lucide-react';
...
const Layout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isLandingRoute) {
      root.style.setProperty('--background', '#f3f8ff');
      root.style.setProperty('--surface', '#ffffff');
      root.style.setProperty('--primary', '#1e90ff');
      root.style.setProperty('--text-primary', '#0f2d66');
      root.style.setProperty('--text-secondary', '#385173');
      root.style.setProperty('--border', '#d4e7ff');
  ...
  }, [palette, themeMode, isLandingRoute]);
      ...
      {isAppReady && (
        <aside className="hidden md:flex fixed left-0 top-0 h-full w-64 border-r border-app bg-surface z-40 flex-col">
          ...
          >
            ...
            <h1 className="text-2xl font-black text-primary-app mt-1">DhandaX Tools</h1>
          </button>
        ...
        <header className="h-16 bg-surface border-b border-app px-4 md:px-6 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-sm md:text-base font-bold">DhandaX Tools</h2>
