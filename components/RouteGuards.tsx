...
export const ProtectedRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const profileReady = Boolean(onboardingCompleted || (phoneNumber?.trim() && selectedProfessionId?.trim()));

  if (!authResolved) {
    return (
      <div className="max-w-xl mx-auto bg-surface border border-app rounded-2xl p-6 text-center space-y-3">
        <h1 className="text-xl font-black">Checking Login...</h1>
        <p className="text-sm text-subtle">Please wait while we verify your session.</p>
      </div>
    );
  }
  ...
  if (!isAdmin && !profileReady) return <Navigate to={ROUTES.setup} replace />;
