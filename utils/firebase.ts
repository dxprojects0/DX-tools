...
const marketingUserRef = (codeLower: string) => doc(db, 'marketingUsers', codeLower);
...
export const savePlanAndPhone = async (uid: string, plan: UserPlan, phoneNumber?: string) => {
  ...
  const planApprovedByAdmin = Boolean(profile?.planApprovedByAdmin && normalizedPlan === normalizePlan(profile?.plan));
  await Promise.all([
    setDoc(
      legacyUserRef(uid),
      sanitizeForFirestore({
        uid,
        plan: normalizedPlan,
        planStatus: normalizedPlan === 'free' ? 'active' : 'pending',
        lastLogin: serverTimestamp(),
        updatedAt: serverTimestamp(),
      }),
      { merge: true },
...
export const saveMarketingAccessUser = async (payload: {
  businessName: string;
  code: string;
  contactName?: string;
  ...
  if (!payload.createdByUid) throw new Error('Admin UID is required.');
  const codeLower = payload.code.trim().toLowerCase();
  if (!codeLower) throw new Error('Access code is required.');

  await Promise.all([
    setDoc(
      marketingUserRef(codeLower),
      sanitizeForFirestore({
        code: payload.code.trim(),
        codeLower,
        businessName: payload.businessName.trim(),
        contactName: payload.contactName || null,
        contactEmail: payload.contactEmail ? payload.contactEmail.toLowerCase() : null,
    ...
    setDoc(
      adminManagementRef('marketing'),
      sanitizeForFirestore({
        lastCodeLower: codeLower,
        lastUpdatedBy: payload.createdByUid,
        updatedAt: serverTimestamp(),
      }),
      { merge: true },
    ),
...
export const getMarketingAccessByCode = async (code: string) => {
  const codeLower = code.trim().toLowerCase();
  if (!codeLower) return null;
  const directSnap = await getDoc(marketingUserRef(codeLower));
  if (directSnap.exists()) return directSnap.data();

  // Legacy fallback for existing entries that were saved under users/{uid}/dashboard/data.
  const q = query(collectionGroup(db, 'dashboard'), where('marketingCode.codeLower', '==', codeLower), limit(1));
  const snap = await getDocs(q);
  if (snap.empty) return null;
  return snap.docs[0].data().marketingCode || null;
};

export const fetchAdminAnalyticsData = async () => {
  const [profileSnap, paidInterestsSnap, adminAccountsSnap] = await Promise.all([
    getDocs(collectionGroup(db, 'profile')),
    getDocs(collection(db, 'PaidInterests')),
    getDocs(collection(db, 'adminAccounts')),
  ]);

  const users = profileSnap.docs.map((docItem) => {
    const data = docItem.data() as any;
    const email = (data.email || '').toLowerCase();
    const role = deriveRoleFromEmail(email);
    const uid = docItem.ref.parent.parent?.id || data.uid || docItem.id;
    return {
      id: uid,
      uid,
      email,
  ...
  const paidInterestUsers = paidInterestsSnap.docs.map((item) => ({
    id: item.id,
    ...(item.data() as any),
  }));
